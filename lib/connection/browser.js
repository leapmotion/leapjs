...
BrowserConnection.prototype.setupSocket = function() {
  ...
  try {
      ...
      if (/An insecure WebSocket connection may not be initiated from a page loaded over HTTPS/.test(err.message)) {
          alert("To enable Leap Motion Controller in your version of Google Chrome, follow these instructions:\n" +
                "\n" +
                "1. Go to chrome://flags/\n" +
                "2. Search for \"Allow insecure WebSocket from https origin\"\n" +
                "3. Click Enable.\n" +
                "4. Click \"Relaunch Now\" in the bottom to restart Chrome.\n" +
                "\n" +
                "Sorry for the inconvenience, we're working on a solution!");
      }
  }
}
