...
/**
  ...
  All 'deviceAttached' events will have a paired 'deviceRemoved' event, and all 'deviceStreaming'
  events will have a paired 'deviceStopped' event.  
  'deviceAttached' will be fired immediately after 'connect' if a device is attached when the connection is established, and whenever a device is plugged in thereafter. 'deviceStreaming' behaves the same way, firing after 'deviceAttached' for devices which are streaming when the connection is established.  When shutting
  down, all connected devices will have 'deviceStopped' & 'deviceRemoved' events.
*/
Controller.prototype.setupConnectionEvents = function() {
  ...
  this.connection.on('deviceEvent', function(evt) {
    ...
    if( evt.state.changed == 'attached' ) {
	  ...
	  if( evt.state.isStreaming ) {
	    outEvent = 'deviceStreaming'
		controller.emit('deviceConnected'); //This should be considered depricated and avoided
	  }
	  else {
	    outEvent = 'deviceStopped';
		controller.emit('deviceDisconnected');  //This should be considered depricated and avoided
	  }
	}
    controller.emit(outEvent, evt.state);
	
	//when we start, we will only recieve a single 'deviceEvent' message - we must translate this into
	//both deviceAttached & deviceStreaming if we are connected while a device is already streaming
	if( outEvent == 'deviceAttached' && evt.state.isStreaming ) {
	  controller.emit('deviceStreaming', evt.state);
	}
  });
}
