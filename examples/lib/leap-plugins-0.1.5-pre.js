/*    
 * LeapJS-Plugins  - v0.1.4 - 2014-05-05    
...
(function() {
  ...
  handHold = function() {
    ...
    interFrameData = {};
    dataFn = function(hashOrKey, value) {
      var key, _name, _results;
      interFrameData[_name = this.id] || (interFrameData[_name] = []);
      if (value) {
        return interFrameData[this.id][hashOrKey] = value;
      } else if (toString.call(hashOrKey) === '[object String]') {
        return interFrameData[this.id][hashOrKey];
      } else {
        _results = [];
        for (key in hashOrKey) {
          value = hashOrKey[key];
          if (value === void 0) {
            _results.push(delete interFrameData[this.id][key]);
          } else {
            _results.push(interFrameData[this.id][key] = value);
          }
        }
        return _results;
      }
